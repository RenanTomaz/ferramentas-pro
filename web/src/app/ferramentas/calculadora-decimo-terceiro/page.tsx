...
export default function Page() {
  ...
  const calcular = () => {
    try {
      ...
      ((p, i, t) => {
        const total = (p / 12) * i; resultado = { total: Number(total.toFixed(2)), itens: [{ label: 'Salário Base', valor: p, tipo: 'info' }, { label: 'Meses Proporcionais', valor: i, tipo: 'quantidade' }] };
  ...
  return (
    <main className={`relative min-h-screen bg-[#020617] py-12 px-6 font-sans text-white transition-all duration-500 ease-in-out ${isVisible && !isExiting ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-indigo-900/10 blur-[150px] rounded-full pointer-events-none z-0"></div>
      ...
      <div className="relative z-10 max-w-2xl mx-auto">
        <header className="mb-12 text-center">
          <button onClick={handleBack} className="group mb-8 flex items-center gap-3 mx-auto bg-white/5 hover:bg-white/10 border border-white/10 px-8 py-3 rounded-full transition-all active:scale-95">
            <span className="text-slate-400 group-hover:text-indigo-400 transition-colors text-[10px] font-black uppercase tracking-[0.3em]">Voltar</span>
          </button>
          <div className="block bg-indigo-500/10 border border-indigo-500/20 px-4 py-1 rounded-full text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-4 italic w-fit mx-auto">
            FerramentasPro
          </div>
          <h1 className="text-4xl sm:text-5xl font-black tracking-tighter italic uppercase leading-none">{ "13º Salário" }</h1>
          <div className="h-1 w-20 bg-indigo-600 mx-auto rounded-full mt-6 shadow-[0_0_15px_rgba(59,130,246,0.5)]"></div>
        </header>
        ...
        <div className="bg-slate-900/40 backdrop-blur-xl rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl p-8 sm:p-12">
            ...
            <div className="mb-6 text-left">
              ...
              <input 
                ...
                className="w-full p-4 bg-slate-900/50 border border-white/10 rounded-2xl outline-none focus:border-indigo-500 transition-all text-white font-mono" 
              />
            </div>
            <div className="mb-6 text-left">
              <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3 italic">Meses Trabalhados</label>
              <input 
                type="number" step="0.01" placeholder="0,00"
            ...
            {detalhes && (
              <div className="mt-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-indigo-500/10 p-10 rounded-[2rem] text-center border border-indigo-500/20 mb-8 relative overflow-hidden">
                  <span className="text-indigo-400 font-black uppercase text-[10px] tracking-[0.3em] mb-4 block italic">Resultado Final</span>
                  <div className="text-5xl sm:text-6xl font-black text-white tracking-tighter">
                    { "calculadora-decimo-terceiro".includes("margem") || "calculadora-decimo-terceiro".includes("roi") || "calculadora-decimo-terceiro".includes("alcool") 
