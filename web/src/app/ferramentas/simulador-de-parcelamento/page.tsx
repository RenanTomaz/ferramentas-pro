...
export default function Page() {
  ...
  return (
    <main className={`relative min-h-screen bg-[#020617] py-4 px-4 font-sans text-white transition-all duration-500 ease-in-out ${isVisible && !isExiting ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-rose-900/10 blur-[120px] rounded-full pointer-events-none z-0"></div>
      ...
      <div className="relative z-10 max-w-xl mx-auto">
        <header className="relative mb-3 text-center pt-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-black tracking-tighter italic uppercase leading-none mb-1">{ "Parcelamento" }</h1>
          <div className="h-1 w-12 bg-rose-600 mx-auto rounded-full shadow-[0_0_10px_rgba(59,130,246,0.5)] mb-3"></div>
        ...
        <div className="bg-slate-900/40 backdrop-blur-xl rounded-3xl border border-white/10 overflow-hidden shadow-2xl p-5 sm:p-8">
            ...
            <div className="mb-3 text-left">
              <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Valor à Vista (R$)</label>
              <input 
                type="number" step="0.01" placeholder="0,00"
                onChange={(e) => setP(Number(e.target.value))} 
                className="w-full p-3 bg-slate-900/50 border border-white/10 rounded-xl outline-none focus:border-rose-500 transition-all text-white font-mono text-sm" 
              />
            </div>
            <div className="mb-3 text-left">
              <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Taxa de Juros Total (%)</label>
              <input 
                type="number" step="0.01" placeholder="0,00"
            ...
            <div className="mb-3 text-left">
              <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1 italic">Nº de Parcelas</label>
              <input 
                type="number" step="0.01" placeholder="0,00"
            ...
            {detalhes && (
              <div className="mt-6 animate-in fade-in slide-in-from-bottom-2 duration-400">
                <div className="bg-rose-500/10 p-5 rounded-2xl text-center border border-rose-500/20 mb-4 relative overflow-hidden">
                  <span className="text-rose-400 font-black uppercase text-[8px] tracking-[0.2em] mb-1 block italic">Resultado</span>
