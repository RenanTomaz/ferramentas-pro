...
export default function Page() {
  ...
  const calcular = () => {
    try {
      ...
      ((p, i, t) => {
        const inss = Number((p * 0.11).toFixed(2)); const irrf = Number((p * 0.075).toFixed(2)); const liquido = p - inss - irrf; resultado = { total: Number(liquido.toFixed(2)), itens: [{ label: 'Salário Bruto', valor: p, tipo: 'positivo' }, { label: 'Desconto INSS (11%)', valor: inss, tipo: 'negativo' }, { label: 'Desconto IRRF (7.5%)', valor: irrf, tipo: 'negativo' }] };
  ...
  return (
    <main className={`relative min-h-screen bg-[#020617] py-12 px-6 font-sans text-white transition-all duration-500 ease-in-out ${isVisible && !isExiting ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
      ...
      <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] bg-emerald-900/10 blur-[150px] rounded-full pointer-events-none z-0"></div>
      ...
      <div className="relative z-10 max-w-2xl mx-auto">
        <header className="mb-12 text-center">
          <button onClick={handleBack} className="group mb-8 flex items-center gap-3 mx-auto bg-white/5 hover:bg-white/10 border border-white/10 px-8 py-3 rounded-full transition-all active:scale-95">
            <span className="text-slate-400 group-hover:text-emerald-400 transition-colors text-[10px] font-black uppercase tracking-[0.3em]">Voltar</span>
          </button>
          <div className="block bg-emerald-500/10 border border-emerald-500/20 px-4 py-1 rounded-full text-[10px] font-black text-emerald-400 uppercase tracking-[0.3em] mb-4 italic w-fit mx-auto">
            FerramentasPro
          </div>
          <h1 className="text-4xl sm:text-5xl font-black tracking-tighter italic uppercase leading-none">{ "Salário Líquido" }</h1>
          <div className="h-1 w-20 bg-emerald-600 mx-auto rounded-full mt-6 shadow-[0_0_15px_rgba(59,130,246,0.5)]"></div>
        </header>
        ...
        <div className="bg-slate-900/40 backdrop-blur-xl rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl p-8 sm:p-12">
            ...
            <div className="mb-6 text-left">
              ...
              <input 
                ...
                className="w-full p-4 bg-slate-900/50 border border-white/10 rounded-2xl outline-none focus:border-emerald-500 transition-all text-white font-mono" 
              />
            </div> 
            ...
            {detalhes && (
              <div className="mt-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="bg-emerald-500/10 p-10 rounded-[2rem] text-center border border-emerald-500/20 mb-8 relative overflow-hidden">
                  <span className="text-emerald-400 font-black uppercase text-[10px] tracking-[0.3em] mb-4 block italic">Resultado Final</span>
                  <div className="text-5xl sm:text-6xl font-black text-white tracking-tighter">
                    { "simulador-clt-vs-pj".includes("margem") || "simulador-clt-vs-pj".includes("roi") || "simulador-clt-vs-pj".includes("alcool") 
